"""Fetch authoritative per-request usage from Deepgram's management API.

Deepgram returns `response.details.usd` per request via
`GET /v1/projects/{project_id}/requests/{request_id}`. Streaming sessions
correspond to a single `request_id` each (verified). Settlement lag is
seconds (observed against the live account), so a ~30s delay after run
close is enough. If the management key or project id is missing, or the
call fails, we log and leave locally-computed cost as the display value.
"""
...
async def schedule_reconciliation(
    ...
    """Wait `delay_seconds` then reconcile. Safe to fire-and-forget."""
    try:
...
async def reconcile_run(run_id: str) -> int:
    """Fetch each captured Deepgram request_id and append a `provider.usage` event.

    Returns the number of `provider.usage` events written.
    """
    ...
    async with SessionLocal() as session:
        ...
        pending = [(rid, kind) for rid, kind in captured if rid not in already_reconciled]
        if not pending:
            return 0
...
async def _load_state(session, run_id: str) -> tuple[list[tuple[str, str]], set[str]]:
    """Return (captured request_ids, already-reconciled request_ids)."""
    result = await session.execute(
        select(TraceEventRecord.event_type, TraceEventRecord.payload).where(
            TraceEventRecord.run_id == run_id,
            TraceEventRecord.event_type.in_(["provider.request_id", "provider.usage"]),
        )
    )
    ...
    for event_type, payload in result.all():
        ...
        if not request_id:
            continue
        if event_type == "provider.request_id":
            captured.append((str(request_id), str(payload.get("kind") or "unknown")))
        else:
...
def _parse_detail(kind: str, request_id: str, detail: dict[str, Any]) -> dict[str, Any] | None:
    ...
    usd = details.get("usd")
    if usd is None:
        # Still pending — retry cycle can pick this up later.
        return None
    ...
    if kind == "stt":
        ...
        payload["characters"] = sum(int(s.get("characters") or 0) for s in segments)
        if segments:
